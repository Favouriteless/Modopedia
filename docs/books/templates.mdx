...
The only thing required to make a new template is a **template.json**. It should be located in a **resource pack** at `assets/<namespace>/modopedia/templates/<template_id>.json` where `namespace` is the ID of your mod or namespace of your modpack, and `template_id` is the ID of your template. Both of these need to follow Minecraft's [ResourceLocation rules](https://minecraft.wiki/w/Resource_location). When referring to your template you should use its full ID in the format `namespace:template_id`. 
